...
$postId = $_GET['id'] ?? null;
if (!$postId) {
    header("Location: /my-posts/?msg=Post ID not provided");
    exit;
}
...
if (!$post || $post['user_id'] != $userId) {
    header("Location: /my-posts/?msg=Unauthorized access");
    exit;
}
...
if ($result) {
    header("Location: /my-posts/?msg=Post moved to draft successfully");
} else {
    header("Location: /my-posts/?msg=Error updating post status");
}
exit;
