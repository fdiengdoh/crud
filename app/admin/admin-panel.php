      ...
      <tbody>
        <?php foreach ($pendingComments as $comm): ?>
          <tr>
            ...
            <td>
              <a href="<?= $link->getUrl('/admin/flag-comment') ?>/?action=approve&id=<?= $comm['id']; ?>" class="btn btn-success btn-sm">Approve</a>
              <a href="<?= $link->getUrl('/admin/flag-comment') ?>/?action=delete&id=<?= $comm['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this comment?');">Delete</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
